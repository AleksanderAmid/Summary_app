...
#   3. gemma3:12b-it-q4_K_M model (~8 GB download via ollama pull)
#   4. PyMuPDF             (pip, optional PDF support)
...
$MODEL_TAG = "gemma3:12b-it-q4_K_M"
...
Write-Step "Step 1/4 - Python 3.10+"
...
if ($py) {
    ...
    if (Test-Winget) {
        ...
        Start-Process $dst -ArgumentList "/quiet InstallAllUsers=0 PrependPath=1 Include_test=0" -Wait
...
# ---------------------------------------------------------------- Model
Write-Step "Step 3/4 - Summarization model ($MODEL_TAG)"
...
# ---------------------------------------------------------------- PyMuPDF
Write-Step "Step 4/4 - PyMuPDF (PDF support)"
...
& cmd /c "$py -c `"import fitz`" >nul 2>&1"
if ($LASTEXITCODE -eq 0) {
    Write-Ok "PyMuPDF already installed"
} else {
    Write-Info "Installing PyMuPDF via pip..."
    & cmd /c "$py -m pip install --quiet pymupdf 2>&1"
    if ($LASTEXITCODE -eq 0) { Write-Ok "PyMuPDF installed" }
    else { Write-Fail "PyMuPDF install failed - PDFs will be disabled (app still works); try: $py -m pip install pymupdf" }
}

Write-Host ""
Write-Host "Setup complete. Start the app with run_app.bat" -ForegroundColor Green
